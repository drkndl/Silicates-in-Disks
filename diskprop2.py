...
def get_Sigma0(Mdust, p, rc, R_in, R_out):
	
	"""
	Calculates the surface density
	"""
	
	den1 = np.exp(-(R_out / rc)**(2-p))
	den2 = np.exp(-(R_in / rc)**(2-p))
	Sigma0 = (Mdust * (p-2)) / (2 * np.pi * rc.to(u.cm)**2 * (den1 - den2))
	# Sigma0 = (Mdust * (p-2)) / (2 * np.pi * rc.to(u.cm)**2)
	
	return Sigma0
	
def surface_density(Sigma0, r, rc, p):
    ...
    # Sigma = Sigma0 * (r / rc)**(p) * np.exp(-(r / rc)**(2-p))
    Sigma = Sigma0 * (r / rc)**(p)
    return Sigma
...
def main():
	...
	Sigma = surface_density(Sigma0, r_arr, Rc, e)
	plt.semilogy(r_arr, Sigma, label = r"Surface density $\Sigma$ [$gm/cm^2$]")
	plt.legend()
	plt.show()
