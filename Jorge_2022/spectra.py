...
dist_pc = 4.84814e-6  			            # 1 AU (distance to the Sun) in parsec
...
def plot_spectra(lamda, summ, solid_name, rv, fmax, Rmin, Rmax):
	...
	plt.plot(lamda, summ)
	plt.xlabel(r'$\lambda$ ($\mu$m)')
	plt.ylabel('Flux (erg / (Hz s sr))')
	plt.title(r'Spectrum {0} r={1} $\mu$m $f_{{max}}$={2} R={3}-{4} AU'.format(latex_name(solid_name), rv, fmax, Rmin.value, Rmax.value))
	plt.savefig("Spectra/Spectrum_{0}_r{1}_f{2}_R{3}-{4}.png".format(solid_name, rv, fmax, Rmin.value, Rmax.value))
	plt.show()
...
def hankel_transform(F_map, R_arr, lamda, wl, B, wl_array):
	...
	bessel = j0(2.0 * np.pi * R_arr * B / (lamda * 10**-6))           						# Shape: (NPOINT, lsize)
	inter_flux = 2.0 * np.pi * np.trapz(R_arr * bessel * F_map, x = R_arr, axis = 0)   		# Shape: (lsize,)
	inter_flux_abs = np.abs(inter_flux)
...
def main():
	...
	lmin = 4.0 * u.micron 						  # Lower limit of wavelength (microns)
	...
	B = np.arange(20.0, 130.0, 2.0) * u.m         # 1D array of baselines (m)
	B1 = 50.0 * u.m                               # A single baseline value (m)
	B_small = np.linspace(20.0, 130.0, 5) * u.m   # 1D array of a few baselines to plot correlated flux against wavelengths (m)
	...
	intflux_sum = np.zeros(lsize) * u.erg / (u.Hz * u.s * u.sr)
	...
	plt.savefig("Spectra/Overall_spectrum_r0.1_R{0}-{1}.png".format(Rmin.value, Rmax.value))
	plt.show()
	
	# Plotting the overall spectrum considering multiple radii together
	Rmin_list = [0.5, 0.75] * u.AU
	Rmax_list = [1.0, 1.28] * u.AU
	intflux_sum_mr = np.zeros(lsize) * u.erg / (u.Hz * u.s * u.sr)
	fig = plt.figure()
	...
	rr = R_arr/dist_pc/3600.0*np.pi/180.0 						# From Interferometric_modelling_demo.ipynb by Jozsef
	
	# Plotting the correlated flux density for a single baseline against wavelengths
	corr_flux_abs = hankel_transform(F_map_sum, rr, lamdas['Mg2SiO4'], wl, B1, wl_array = True)
	
	fig = plt.figure()
	plt.plot(lamdas['Mg2SiO4'], corr_flux_abs)
	plt.xlabel(r'$\lambda$ ($\mu$m)')
	plt.ylabel('Correlated flux')             # !!!!!!!!!!!!!!!!!!!!!!!!!!! CHECK UNITS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	plt.title(r'Correlated flux for rv = 0.1 $\mu$m, B = {0} m'.format(B1))
	plt.savefig('Spectra/Corr_flux_withrr_rv0.1_B{0}.png'.format(B1))
	plt.show()
	...
	for Bl in B_small:
		...
		corr_flux_absB = hankel_transform(F_map_sum, rr, lamdas['Mg2SiO4'], wl, Bl, wl_array = True)
		plt.plot(lamdas['Mg2SiO4'], corr_flux_absB, label = 'B = {0} m'.format(Bl))
	
	plt.xlabel(r'$\lambda$ ($\mu$m)')
	plt.ylabel('Correlated flux')             # !!!!!!!!!!!!!!!!!!!!!!!!!!! CHECK UNITS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	plt.title(r'Correlated flux for multiple baselines, rv = 0.1 $\mu$m')
	plt.legend()
	plt.savefig('Spectra/Corr_flux_withrr_multB_rv0.1.png')
	plt.show()
	...
	plt.savefig("Spectra/Corr_flux_wl{0}.png".format(wl))
	plt.show()
